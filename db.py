...
def record_zone_exit(vehicle, zone, lat, lng):
    """
    يسجل الخروج ويحسب المدة.
    يرجع (entered_at, duration_sec, zone_name) أو None.
    لو ما لقى زيارة مفتوحة بنفس اسم المنطقة، يغلق آخر زيارة مفتوحة
    للمركبة (لأن اسم المنطقة قد يختلف بين حدثي الدخول والخروج).
    """
    now = datetime.now()
    conn = get_conn(); cur = conn.cursor()

    # ١) محاولة التطابق الدقيق بالاسم
    cur.execute(q("""SELECT id, entered_at, zone FROM zone_visits
                  WHERE vehicle=? AND zone=? AND exited_at IS NULL
                  ORDER BY id DESC LIMIT 1"""), (vehicle, zone))
    row = cur.fetchone()

    # ٢) لو ما فيه، أي زيارة مفتوحة لنفس المركبة
    if not row:
        cur.execute(q("""SELECT id, entered_at, zone FROM zone_visits
                      WHERE vehicle=? AND exited_at IS NULL
                      ORDER BY id DESC LIMIT 1"""), (vehicle,))
        row = cur.fetchone()
    ...
    visit_id, entered_at, stored_zone = row[0], row[1], row[2]
    ...
    conn.commit(); cur.close(); conn.close()
    return entered_at, duration, stored_zone


def _norm(v):
    """تحويل أنواع Postgres (Decimal/date/datetime) لأنواع قابلة للتحويل لـ JSON"""
    from decimal import Decimal
    from datetime import date as _date, datetime as _dt
    if isinstance(v, Decimal):
        return float(v)
    if isinstance(v, (_date, _dt)):
        return v.isoformat() if isinstance(v, _dt) else v.strftime("%Y-%m-%d")
    return v
...
def get_speeding_report(date=None, limit=100):
    ...
    try:
        ...
        rows = [dict(zip(cols, [_norm(x) for x in r])) for r in cur.fetchall()]
    finally:
        cur.close(); conn.close()
    return rows
...
def get_zone_report(zone=None, limit=100):
    ...
    rows = [dict(zip(cols, [_norm(x) for x in r])) for r in cur.fetchall()]
    for r in rows:
        r["entered_at"] = str(r["entered_at"]) if r["entered_at"] else None
