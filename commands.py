...
def to_local_str(val, time_only=True):
    """تحويل وقت مخزّن (UTC) إلى نص محلي"""
    if not val:
        return "—"
    try:
        s = str(val).replace("T", " ").split(".")[0]
        dt = datetime.fromisoformat(s) + timedelta(hours=TZ_OFFSET)
...
def cmd_zones():
    ...
    for r in rows[:20]:
        ent = to_local_str(r["entered_at"], time_only=False)
        t += f"\n🚗 <b>{r['vehicle']}</b>\n"
        t += f"   📍 {r['zone']}\n"
        t += f"   ⏱ دخل: {ent}\n"
        if r["exited_at"]:
...
def cmd_inside():
    ...
    for r in inside:
        try:
            ent = datetime.fromisoformat(str(r["entered_at"]).replace("T"," ").split(".")[0])
            dur = fmt_duration((datetime.utcnow() - ent).total_seconds())
        except Exception:
            dur = "—"
        t += f"\n🚗 <b>{r['vehicle']}</b>\n"
        t += f"   🏭 {r['zone']}\n"
...
def cmd_car(query):
    ...
    if zone_rows:
        ...
        for r in zone_rows[:10]:
            ent = to_local_str(r["entered_at"], time_only=False)[:16]
...
def cmd_today():
    ...
    if today_zones:
        ...
        for z in today_zones[:15]:
            t += f"  🚗 <b>{z['vehicle']}</b>\n"
            t += f"     📍 {z['zone']}\n"
            t += f"     ⏱ دخل: {to_local_str(z.get('entered_at'))}\n"
            if z.get("exited_at"):
                t += f"     ⏱ خرج: {to_local_str(z['exited_at'])}\n"
                t += f"     ⏳ المدة: <b>{fmt_duration(z['duration_sec'])}</b>\n"
            else:
                try:
                    ea = datetime.fromisoformat(str(z['entered_at']).replace("T"," ").split(".")[0])
                    elapsed = (datetime.utcnow() - ea).total_seconds()
...
def cmd_yesterday():
    ...
    if yest_zones:
        ...
        for z in yest_zones[:10]:
            t += f"  🚗 <b>{z['vehicle']}</b>\n"
            t += f"     📍 {z['zone']}\n"
            ent = to_local_str(z.get('entered_at'))
            if z.get("duration_sec"):
                t += f"     ⏱ دخل: {ent} → خرج: {to_local_str(z.get('exited_at'))}\n"
                t += f"     ⏳ المدة: <b>{fmt_duration(z['duration_sec'])}</b>\n"
            else:
                t += f"     ⏱ دخل: {ent}\n"
            t += "\n"
...
def cmd_date(arg):
    ...
    if day_zones:
        ...
        for z in day_zones[:10]:
            ...
            ent = to_local_str(z.get('entered_at'))
            if z.get("duration_sec"):
                t += f"     ⏱ {ent} → {to_local_str(z.get('exited_at'))} (<b>{fmt_duration(z['duration_sec'])}</b>)\n"
            else:
                t += f"     ⏱ دخل {ent} — 🔴 لا يزال\n"
