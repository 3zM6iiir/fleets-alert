...
def extract_fields(data):
    ...
    return {
        ...
        "zone": zone,
        "event_time": data.get("time") or data.get("created_at") or "",
...
def handle_zone_enter(f):
    ...
    t += f"🕐 وقت الدخول: {f['event_time'] or datetime.now().strftime('%H:%M:%S')}\n"
...
def handle_zone_exit(f):
    ...
    now_txt = f["event_time"] or datetime.now().strftime("%H:%M:%S")
