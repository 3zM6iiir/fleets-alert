...
def format_alert(data):
    ...
    if plate and str(plate).strip() and str(plate) != "None":
        text += f"🔢 اللوحة: <b>{plate}</b>\n"
    if driver and str(driver).strip() and str(driver) != "None" and str(driver) != "":
    ...
    if address and str(address).strip() and str(address) != "None":
...
def test():
    test_data = {"name":"تورس س ر ح 1678 - 2","plate_number":"أ ب ج 1234","event":"overspeed","speed":145,"lat":20.4306516,"lng":44.9318583,"message":"تجاوز السرعة (120 كيلومتر في الساعة)","address":"طريق الخميس السليل، الشرافة، محافظة وادي الدواسر"}
